...
Properties {
    ...
    _Rotation ("RotationY", Range(0, 360)) = 0
...
SubShader {
    ...
    Pass {
        ...
        //ÑØXÖáÐý×ª
	    float3 DoTwistX(float3 Position, float degrees)
	    {
	    	float3 result = 0;
	    	float alpha = degrees * UNITY_PI / 180.0;
	    	float sina, cosa;
	    	sincos(alpha, sina, cosa);
	    	result.x = Position.x;
	    	result.y = Position.y * cosa + sina * Position.z;      
	    	result.z = -Position.y * sina + cosa * Position.z;
		    return result;
	    }

    	//ÑØZÖáÐý×ª
    	float3 DoTwistZ(float3 Position, float degrees)
	    {
	    	float3 result = 0;
	    	float alpha = degrees * UNITY_PI / 180.0;
	    	float sina, cosa;
	    	sincos(alpha, sina, cosa);
	    	result.x = Position.x * cosa + sina * Position.y;
		    result.y = -Position.x * sina + cosa * Position.y;
		    result.z = Position.z;
		    return result;
	    }
